...
async def _parse_forward_message(
    ...
    Returns:
        (AI用纯文本, 结构化消息列表)
        结构化消息: [{"sender": str, "content": str, "images": [str]}]
    """
    if depth >= MAX_FORWARD_DEPTH:
        return "[嵌套转发消息，层级过深]", []
            ...
            if seg_type == "text":
                ...
                if inline_content and isinstance(inline_content, (list, str)):
                    if isinstance(inline_content, str):
                        try:
                            import json as _json
                            inline_content = _json.loads(inline_content)
                        except Exception:
                            inline_content = None
                    if inline_content:
