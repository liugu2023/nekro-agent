...
- 任务会提供一个可写的插件工作副本路径和插件自检命令。候选代码必须先写入该工作副本，再由你在 CC 沙盒里运行该自检命令；只在最终回复里粘贴代码不算交付。
- 自检命令执行的是静态检查（语法、导入可用性、插件结构），不会运行插件代码；完整加载检查在用户确认应用提案时由后端执行，因此静态自检通过不代表运行时一定无错。
- 自检通过后，优先调用内部网关 `/proposals` 创建写入提案。不要在自检前创建 proposal。若网关不可用，也必须保证工作副本路径里的文件已经是最终候选代码。
- 包形式插件调用 `/check` 或 `/proposals` 时，`files` 只提交当前仍存在的完整 `.py` 文件；已删除的包内文件必须通过 `deleted_files` 字符串数组提交，不能仅从 `files` 中省略。
...
- 内部网关只允许 `/version`、`/files`、`/file?path=...`、`/check`（仅静态检查）、`/proposals`，不要尝试直接 apply 或访问其他宿主机路径。
