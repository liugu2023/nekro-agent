...
> **修订（2026-07）：安全模型与运行行为已按以下原则落地，若与下文原始设计冲突，以本节为准。**
>
> 1. **迭代期自检 = 静态检查（static 级别）**：CC 沙盒自检命令与宿主机每轮复核均使用 `static` 级别（AST 语法、模块级导入可用性、`plugin = NekroPlugin(...)` 实例定义、挂载函数 async 约束），绝不执行候选代码；候选目录永不进入 `sys.path`，导入验证逐级 `find_spec` 且数据目录下的模块一律跳过加载。
> 2. **执行型检查（smoke）只在 apply 时刻**：用户审阅 diff 并确认应用后，后端才运行完整加载检查（等同于信任插件真实运行）。
> 3. **内部网关 `/check` 钳制为 static**：token 泄漏不再构成宿主机代码执行原语；网关 token 在每次新建沙盒容器时轮换。
> 4. **apply 并发保护**：提案记录 `before_sha256`，应用前与磁盘内容比对，不一致则拒绝。
> 5. **任务生命周期**：取消排队任务不中断活动任务；服务重启时回收非终态任务并清理超期任务/提案文件；任务结束时自动丢弃遗留 pending 提案；生成请求限制 512KB、排队上限 3 个；单文件版本历史保留最近 50 个。
